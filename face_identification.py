...
input = cv2.imread("Inputs/a1.jpg")
...
for result in results:
    ...
    for person in face_bank:
        ...
        if dist<threshold:
            cv2.putText(input, person["name"], (int(result.bbox[0]) - 10, int(result.bbox[1] - 10)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            break

    else:
        cv2.putText(input, "Unknown", (int(result.bbox[0]) - 10, int(result.bbox[1] - 10)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
